...
function VatConfigRow({ jobId }: { jobId: string }) {
  ...
  // callback) — matches the pattern the rest of this file already uses for
  // its fetch effects, which the stricter set-state-in-effect lint rule
  // requires. save() below performs its own separate fetch-and-refresh
  // rather than sharing this closure.
  useEffect(() => {
    let cancelled = false
    async function doLoad() {
      try {
        ...
        if (cancelled) return
        setTreatment(data.treatment ?? { mode: 'not_configured', ratePct: null })
      } catch {
        if (!cancelled) setTreatment({ mode: 'not_configured', ratePct: null })
      } finally {
  ...
  const save = async () => {
    ...
    try {
      ...
      setTreatment(draftMode === 'rate' ? { mode: 'rate', ratePct: Number(draftRate) } : { mode: 'zero_rated', ratePct: null })
    } catch {
