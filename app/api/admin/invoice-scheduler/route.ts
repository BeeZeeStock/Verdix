...
export async function GET(req: NextRequest) {
  ...
  for (const row of dueRows) {
    ...
    try {
      ...
      // ── Compute and add overage line items (period invoices only) ─────────
      if (row.invoice_type === 'period') {
        ...
        if (meterConfigs && meterConfigs.length > 0) {
          ...
          // ── Fallback: legacy single org-level pull endpoint ───────────────
          ...
          if (pullRes.ok) {
            ...
            if (aggregateUnits > 0) {
              const overageAmount = Math.round(
                computeMetricOverage(aggregateUnits, terms.overage_tiers ?? [], includedUnits) * 100
              ) / 100
              ...
              if (overageAmount > 0) {
                ...
                await stripe.invoiceItems.create({
                  ...
                  description: overageDesc,
                  metadata: {
                    metric_source: 'client_pull',
                    total_units:   String(aggregateUnits),
                    verdix_job:    row.job_id,
                  },
                })
              }
      ...
      results.push({ id: row.id, ok: true, stripe_invoice_id: inv.id })
    } catch (err) {
