...
All commercial-reasoning AI calls — contract extraction (`lib/contract-extractor.ts`), propose-rule/interpret-rule (`app/api/jobs/[id]/propose-rule|interpret-rule`), and partner agreement/invoice/reconciliation (`lib/partner-reconciler.ts`) — run on **Sonnet** via `lib/ai-client.ts`'s `getAIClient()`. Meter-key matching runs on **Haiku** via `getFastAIClient()`. `getReasoningAIClient()` (Opus, EU Bedrock, adaptive thinking, configurable effort) is fully implemented — separate 280s/no-retry timeout budget, 290s route `maxDuration` headroom, 16k–20k token budgets, thinking-block stripping, thinking-token telemetry — but **not used by any production call site**.

**Do not switch a call site to the reasoning tier without a new benchmark.** A live two-round A/B (2026-08-20/21, TEST-PAY-002 — see commit range `3860fe6e`–`b4b28eb4`) found Opus added no measurable factual-accuracy benefit over Sonnet: both models captured identical contract facts and produced the identical canonical blocker set after provenance gating, at ~4.25× Opus's latency and a comparable multiple in tokens. Opus's only behavioral difference was offering `verdix_recommends` guesses from commercial-plausibility reasoning (e.g. "otherwise credits could be forfeited, contradicting the undertaking") where Sonnet correctly left the same field `decision_required` with no value. **More reasoning is not automatically better for contract execution — preserving contractual silence is a desirable model behavior for this product's risk profile.** Revisit only with new evidence of a material accuracy gain on a specific pipeline, not a general "upgrade the model" instinct.

**AI confidence is not provenance — enforced structurally, not by convention.** `lib/types.ts`'s `FieldProvenance` (`contract_derived` | `verdix_recommends` | `reviewer_policy`) and `lib/commercial-rule-status.ts`'s `isProvenanceResolved()` are the sole gate on whether a billing-impacting field can become executable. A field resolves only when `contract_derived` (the contract states/unambiguously implies it) or `reviewer_policy` (a human explicitly confirmed or chose it) — `verdix_recommends` stays review-blocking regardless of how confident or well-reasoned the model's justification is, and regardless of whether it returned a fully concrete value. See `lib/credit-application-rule.ts` (reference implementation) and its test file (regression matrix: recommendation-never-resolves, contract-derived/reviewer-policy-do, provenance-survives-a-later-unrelated-confirm). Never reintroduce a per-route "is this value present/concrete" check as a substitute for this gate.
