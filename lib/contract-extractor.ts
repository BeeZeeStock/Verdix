...
- base_fee_proration (on base_monthly_fee/base_annual_fee) and each additional_recurring_fees[].proration: only populate when the contract's start date falls MID-CYCLE relative to a fee that resets on fixed CALENDAR boundaries (e.g. contract starts 17 Aug but the fee is described as billing "each calendar month" or "each calendar quarter", not simply anchored to the contract's own start-date anniversary — most contracts use the latter, which has no partial-period question at all). When it does apply: { "reset_anchor": "calendar", "prorate_partial_periods": true|false|"unclear", "requires_confirmation": <boolean>, "confirmation_reason": "<string or null>", "source_clause": "<verbatim or paraphrased, or null>" }. Whether a partial first/last calendar period bills in full or prorated is very rarely stated explicitly for a flat recurring fee — unless the contract text actually says so, set prorate_partial_periods to "unclear" and requires_confirmation to true. NEVER default to full-charge or to proration as a "reasonable assumption" — this mirrors the exact discipline already required for a metric minimum commitment's own prorate_partial_periods field, generalized to the base/recurring fees themselves. Leave proration null/omitted entirely for the (much more common) case of contract-start-anchored billing, where this question doesn't arise.
