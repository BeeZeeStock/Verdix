...
import { compileExecutableCommercialMechanisms, resolveExtractedSemanticInputKeys } from './commercial-mechanism-compiler'
...
- additional_recurring_fees: array of secondary recurring fee components that exist alongside base_monthly_fee. Each entry: { "fee_label": "<name>", "amount": <number>, "description": "<brief note or null>", "billing_frequency": "<cadence or null>", "metric_name": "<string or null>", "rate_per_unit": <number or null>, "required_operational_inputs": <array of strings or null>, "semantic_input_key": "<string or null — see the semantic_input_key rule below>", "unresolved_kind": "unsupported_semantics" | null, "charge_basis_input_key": "<string or null — see the charge_basis_input_key rule below>", "rate_schedule_bands": <array of {"from": <number>, "to": <number or null>, "rate_pct": <number>} objects, or null — see the rate_schedule_bands rule below>, "source_clause": "<verbatim or paraphrased, or null>", "source_sections": <array of {"exact_source_heading": "...", "display_label": "..."} objects, or null — see field_sources' MULTI-SECTION EVIDENCE rule; populate whenever this fee's evidence spans more than one heading> }. Use this when the contract explicitly lists multiple SEPARATE recurring line items with distinct names and amounts.
  FIXED vs. PER-UNIT/VARIABLE RULE — this is a common, high-cost failure mode: a recurring fee is either a FIXED amount charged every billing period regardless of usage, OR a RATE charged per unit of operational activity (per transaction, per request, per completed event) with the actual monthly total depending on volume that doesn't exist yet at extraction time. These are NOT the same shape, and confusing them causes a fee like "€0.38 per issued payment request" to be wrongly treated as a fixed €0.38/month charge multiplied by the contract's billing-cycle count — a number with no relationship to what will actually be owed.
    ...
    - semantic_input_key: ONLY for a PER-UNIT/VARIABLE fee — the canonical operational fact this rate multiplies, chosen from the SAME small set of recognized concepts this system already knows about (do not invent a new name): "issued_payment_request_count" for a fee charged per issued/submitted payment request, "completed_payment_count" for a fee charged per successfully completed payment. If the counted event doesn't clearly match one of these known concepts, leave this null — never guess a name just to fill the field in, and never derive it by reformatting metric_name's own free-text spelling (metric_name stays a display label only; this field is a separate, precise identity judgment).
...
- overage_tiers: usage-based charges above included units. Each tier must have:
  ...
  - semantic_input_key: the canonical operational fact this tier's usage count actually measures, chosen ONLY from the same small set of recognized concepts this system already knows about (never invent a new name): "issued_payment_request_count" when unit_type counts issued/submitted payment requests, "completed_payment_count" when it counts completed payments. Set null when unit_type does not clearly match one of these known concepts — this lets an overage tier and a per-unit fee (additional_recurring_fees) elsewhere in the SAME contract that count the same real-world event resolve to the identical canonical identity, so a reviewer configures one usage source instead of several; never guess this just to make two entries match.
...
export function applyExtractionSafetyNets(terms: ContractTerms, contractText?: string): ContractTerms {
  ...
  // Step 17D.1, item D — validate/canonicalize any explicit
  // semantic_input_key extraction stated (per-unit fees, overage tiers)
  // BEFORE compiling — never inferred from metric_name/unit_type text.
  terms = resolveExtractedSemanticInputKeys(terms)
